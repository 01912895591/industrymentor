...
export function CareerPathCard({ pathway, isSelected = false, onSelect }: CareerPathCardProps) {
  ...
  return (
    <div
      className={cn(
        "group relative flex flex-col justify-between rounded-xl border p-6 sm:p-7 transition-all duration-200 backdrop-blur-xl shadow-sm text-left",
        isSelected
          ? "border-primary bg-card/80 ring-1 ring-primary shadow-md"
          : "border-border/60 bg-card/30 hover:border-border-active hover:bg-card/50 hover:shadow-md"
      )}
    >
      <div className="space-y-4">
        {/* Top Meta Bar */}
        <div className="flex items-start justify-between gap-3">
          <div className="grid h-12 w-12 place-items-center rounded-xl bg-primary/10 border border-primary/20 text-primary shadow-sm transition-transform duration-300 group-hover:scale-105">
        ...
        <div className="rounded-xl border border-border/40 bg-surface-2/40 p-3.5 space-y-1.5">
