...
export interface CertificateVectorProps {
  ...
  qrSrc?: string;
  barcodeSvgInner?: { viewBox: string; inner: string };
  idPrefix?: string;
  className?: string;
  style?: CSSProperties;
}

export function CertificateVector({
  studentName,
  courseTitle,
  issueDate,
  certificateId,
  ...
  qrSrc: propQrSrc,
  barcodeSvgInner: propBarcodeSvgInner,
  idPrefix = "",
  className,
  style,
}: CertificateVectorProps) {
  const [internalQrSrc, setInternalQrSrc] = useState<string>("");
  const [internalBarcode, setInternalBarcode] = useState<{ viewBox: string; inner: string }>({
    viewBox: "0 0 185 80",
    inner: "",
  });
  ...
  useEffect(() => {
    ...
    QRCode.toDataURL(verifyUrl, {
      ...
      .then(setInternalQrSrc)
      .catch((err) => console.error("QR Generation failed", err));
  }, [verifyUrl, propQrSrc]);

  // Code128 Barcode via bwip-js
  useEffect(() => {
    if (propBarcodeSvgInner) return;
    try {
      ...
      setInternalBarcode({ viewBox: vb, inner: cleanInner });
    } catch (e) {
      console.error("Barcode generation error:", e);
    }
  }, [displayCertId, propBarcodeSvgInner]);

  const activeQr = propQrSrc || internalQrSrc;
  const activeBarcode = propBarcodeSvgInner || internalBarcode;

  const brandGradId = `${idPrefix}imBrandGrad`;
  const borderGradId = `${idPrefix}imBorderGrad`;
  const holoGradId = `${idPrefix}imHoloGrad`;
  const sealGradId = `${idPrefix}imSealGrad`;
  const innerClipId = `${idPrefix}imInnerBorderClip`;
  ...
  return (
    ...
    >
      <defs>
        ...
        <linearGradient id={brandGradId} x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        <radialGradient id={sealGradId} cx="40%" cy="40%" r="60%">
      ...
      <rect x="18" y="18" width="964" height="671" fill="none" stroke={`url(#${borderGradId})`} strokeWidth="4" />
      ...
      <g transform="translate(895, 90)">
        ...
        <circle cx="0" cy="0" r="43" fill={`url(#${sealGradId})`} />
      ...
      <g transform="translate(500, 496)">
        <rect x="-100" y="0" width="200" height="9" rx="2" fill={`url(#${holoGradId})`} />
      ...
      <g transform="translate(0, 595)">
        ...
        <g transform="translate(62, -10)">
          {activeBarcode.inner && (
            <svg
              ...
              width="120"
              height="34"
              viewBox={activeBarcode.viewBox}
              dangerouslySetInnerHTML={{ __html: activeBarcode.inner }}
            />
          )}
        ...
        <g transform="translate(495, -45)">
          ...
          {activeQr && <image href={activeQr} x="-25" y="0" width="50" height="50" />}
